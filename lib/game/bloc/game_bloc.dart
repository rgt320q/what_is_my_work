  ...
  GameBloc({GameStateRepository? repository, SharedPreferences? prefs})
    ...
    on<GamePauseRequested>((event, emit) {
      if (state is GameLoaded) {
        final currentState = state as GameLoaded;
        emit(
          GameLoaded(currentState.profile, currentState.user, isPaused: true),
        );
      }
    });
    ...
    on<GameResumeRequested>((event, emit) {
      if (state is GameLoaded) {
        final currentState = state as GameLoaded;
        emit(
          GameLoaded(currentState.profile, currentState.user, isPaused: false),
        );
      }
    });
        ...
        for (int l = 0; l < newLevels.length; l++) {
          ...
          for (int s = 0; s < newLevels[l].stages.length; s++) {
            ...
            for (int t = 0; t < newLevels[l].stages[s].tasks.length; t++) {
              if (l == event.levelIndex &&
                  s == event.stageIndex &&
                  t >= event.taskIndex)
                break;

              newLevels[l].stages[s].tasks[t].isCompleted = true;
              newLevels[l].stages[s].tasks[t].startTime = DateTime.now()
                  .subtract(Duration(minutes: t + 1));
              newLevels[l].stages[s].tasks[t].endTime = DateTime.now().subtract(
                Duration(minutes: t),
              );
            }
          }
